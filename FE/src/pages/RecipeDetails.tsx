    ...
    <>
            ...
            <Swiper className="w-full" direction="horizontal" slidesPerView="auto">
              <SwiperSlide>
                <div className="relative w-full h-full flex shrink-0">
                  ...
                  <img src={`${assetUrl}/${recipe.thumbnail}`} className="w-full h-full object-cover" alt="thumbnail" />
                </div>
              </SwiperSlide>
              {recipe.photos.map((photo) => (
                <SwiperSlide key={photo.id}>
                  <div className="relative w-full h-full flex shrink-0">
                    <div className="gradient-filter absolute w-full h-full bg-[linear-gradient(180deg,rgba(0,0,0,0)40.47%,#000000_81.6%)] z-10" />
                    <img src={`${assetUrl}/${photo.photo}`} className="w-full h-full object-cover" alt="thumbnail" />
                  </div>
                </SwiperSlide>
              ))}
            </Swiper>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="flex shrink-0 w-[50px] h-[50px] rounded-full overflow-hidden">
              <img src={`${import.meta.env.VITE_APP_ASSET_URL}/${recipe.author.photo}`} className="w-full h-full object-cover" alt="avatar" />
            </div>
        ...
        <div className="swiper-tabs w-full overflow-hidden mt-3">
          <div className="swiper-wrapper" id="default-tab" data-tabs-toggle="#default-tab-content" role="tablist" data-tabs-active-classes="shadow-[0_10px_20px_0_#FF4C1C80] !bg-[#FF4C1C]" data-tabs-inactive-classes="!bg-black">
            <Swiper className="w-full" direction="horizontal" spaceBetween={16} slidesPerView="auto" slidesOffsetBefore={20} slidesOffsetAfter={20}>
              {['ingredients', 'tutorials', 'reviews'].map((tab) => (
                <SwiperSlide className="!w-fit pb-[26px]" key={tab}>
                  <button
                    className={`flex items-center gap-[10px] py-3 px-4 rounded-full font-semibold transition-all duration-300 ${
                      activeTab === tab ? '!text-white bg-[#FF4C1C] shadow-[0_10px_20px_0_#FF4C1C80]' : '!text-white bg-black hover:bg-[#FF4C1C] hover:shadow-[0_10px_20px_0_#FF4C1C80] transition-colors duration-300'
                    }`}
                    id={`${tab}-tab`}
                  ...
                  >
                    <img src={`/assets/images/icons/${tab === 'ingredients' ? 'coffee' : tab === 'tutorials' ? 'note-favorite' : 'star'}-white.svg`} className="w-[22px] h-[22px]" alt="icon" />
            ...
            <div className="px-5" id="ingredients" role="tabpanel" aria-labelledby="ingredients-tab">
              <div className="grid grid-cols-2 gap-5">
                {recipe.recipe_ingredients.map((ingredient) => (
                  <IngredientCard key={ingredient.id} ingredient={ingredient.ingredient} />
                ))}
              </div>
            </div>
          )}
      ...
      <div id="BottomNav" className="fixed z-50 bottom-0 w-full max-w-[640px] mx-auto border-t border-[#E7E7E7] py-4 px-5 bg-white/70 backdrop-blur">
        <div className="flex items-center justify-between gap-3">
          ...
          <a target="_blank" href={`${assetUrl}/${recipe.url_file}`} className="py-3 px-5 rounded-full font-semibold text-white text-nowrap transition-all duration-300 shadow-[0_10px_20px_0_#FF4C1C80] bg-[#FF4C1C]">
            Download Now
          </a>
        </div>
      </div>
